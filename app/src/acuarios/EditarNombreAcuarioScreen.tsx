...
type Navegacion = NativeStackNavigationProp<ApplicationStackParamList, 'EditarNombreAcuario'>;
type Ruta = RouteProp<ApplicationStackParamList, 'EditarNombreAcuario'>;

export function EditarNombreAcuarioScreen() {
  ...
  const [nombre, setNombre] = useState(acuario.nombre);
  const [volumenLitros, setVolumenLitros] = useState(
    acuario.volumen_litros !== null ? String(acuario.volumen_litros) : ''
  );
  const [tipoAgua, setTipoAgua] = useState<'DULCE' | 'SALADA' | null>(acuario.tipo_agua);
  ...
  async function manejarGuardar() {
    ...
    setGuardando(true);
    try {
      await editarAcuario(accessToken, acuario.id, {
        nombre: nombre.trim(),
    ...
    <View style={{ flex: 1, backgroundColor: '#F4F1EC' }}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 24 }}>
          ...
          <Boton texto="Guardar cambios" onPress={manejarGuardar} cargando={guardando} />
        </ScrollView>
